...
# IT-сленг
"""
 Сделаем небольшое отклонение в сторону и узнаем некоторые словечки, которые используются в IT-сфере. Конечно, я понимаю, что в разных компаниях и подразделениях свой язык, - опишу те слова, с которыми встречался сам.
Поехали!
 баг (bug) - дефект, отклонение фактического результата теста от ожидаемого
блокер (blocker) - дефект, блокирующий дальнейшее выполнение теста, исправляется в первую очередь (встречал даже такой забавный вариант, как блокатор)
критикал, крит (critical) - дефект, блокирующий UI, можно обойти, используя специальный инструментарий
мажор, мейджер (major) - дефект, влияющий на какую-то функциональную часть приложения
нОрмал, нормАл (normal) - незначительный дефект
лоу (low), тривиал (trivial) - наименее значимый, косметический дефект, исправляется в последнюю очередь (а часто так и остается в длинном to-do списке)
фиксить (fix, fixing) - действие разработчика, направленное на исправление бага
фикс (fix), патч (patch) - часть кода, непосредственно исправляющая дефект (часто в выражении "подлить фикс на сервер")
"""